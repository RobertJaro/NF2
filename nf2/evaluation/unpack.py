...
def load_spherical(save_path, height=None, device=None, resolution=512, **kwargs):
    if device is None:
        device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    state = torch.load(save_path, map_location=device)
    model = nn.DataParallel(state['model'])
    ...
    coords = np.stack(
        np.meshgrid(np.linspace(-height, height, resolution),
                    np.linspace(-height, height, resolution),
                    np.linspace(-height, height, resolution), indexing='ij'), -1)
    spatial_norm = 1
    b = load_coords(model, spatial_norm, state['b_norm'], coords, device, **kwargs)
...
def load_coords(model, spatial_norm, b_norm, coords, device, batch_size=1000, progress=False, compute_currents=False):
    ...
    if (compute_currents or
            isinstance(model, VectorPotentialModel) or (isinstance(model, nn.DataParallel) and isinstance(model.module, VectorPotentialModel)) or \
            isinstance(model, FluxModel) or (isinstance(model, nn.DataParallel) and isinstance(model.module, FluxModel))):
        return _load(coords)
    else:
        with torch.no_grad():
            return _load(coords)
