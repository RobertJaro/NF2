...
field_map = _field_line_endpoint(field_lines).reshape(coords.shape)
...
extent = [0, 360, 0, 180]
fig, axs = plt.subplots(4, 1, figsize=(12, 18), subplot_kw={'projection': aia_map})

im = axs[0].imshow(aia_map.data, origin='lower', cmap=cm.sdoaia193, extent=extent)
plt.colorbar(im, label='Intensity [DN/s]')

cmap = plt.get_cmap('gray')
cmap.set_bad('yellow')
im = axs[1].imshow(field_map, cmap=cmap, origin='lower', extent=extent, vmin=1, vmax=rss)
plt.colorbar(im, label='Endpoint [$R_{\odot}$]')

im = axs[2].imshow(potential_field_map, cmap=cmap, origin='lower', extent=extent, vmin=1, vmax=rss)
plt.colorbar(im, label='Endpoint [$R_{\odot}$]')

im = axs[3].imshow(mag_r_map.data, origin='lower', cmap=cmap, extent=extent, vmin=-500, vmax=500)
...
axs[0].set_ylabel('Carrington Latitude [deg]')
axs[0].set_title('AIA 193')
axs[1].set_title('NF2')
axs[2].set_title('PFSS')
axs[3].set_title('HMI')
