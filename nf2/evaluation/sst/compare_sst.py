...
[ax.plot([y_slice * Mm_per_pixel, y_slice * Mm_per_pixel], [y_range[0], y_range[1]], color='red', linestyle='--') for ax
 in axs]
...
ax = axs[0]
ax.imshow(np.linalg.norm(mfr_1slice_out['b'][y_slice, :, :].to_value(u.G), axis=-1).T, origin='lower', extent=extent,
...
ax = axs[1]
im = ax.imshow(np.linalg.norm(mfr_2slice_out['b'][y_slice, :, :].to_value(u.G), axis=-1).T, origin='lower',
...
ax = axs[0]
ax.imshow(mfr_1slice_out['b'][y_slice, :, :, 2].to_value(u.G).T, origin='lower', extent=extent,
...
ax = axs[1]
im = ax.imshow(mfr_2slice_out['b'][y_slice, :, :, 2].to_value(u.G).T, origin='lower', extent=extent,
               vmin=-100, vmax=100, cmap='RdBu_r')
ax.set_title('NF2 - multi height')
divider = make_axes_locatable(ax)
cax = divider.append_axes('right', size='5%', pad=0.05)
...
plt.tight_layout()
plt.savefig(os.path.join(out_path, 'sst_PIL.png'), dpi=300, transparent=True)
plt.close()
...
ax.imshow(np.linalg.norm(mfr_1slice_out['metrics']['j'].to_value(u.G / u.s), axis=-1).sum(2).T * Mm_per_pixel * 1e8,
          origin='lower', extent=extent, norm=j_norm, cmap='inferno')
ax.set_title('NF2 - single height')
divider = make_axes_locatable(ax)
cax = divider.append_axes('right', size='5%', pad=0.05)
cax.set_axis_off()
...
ax = axs[1]
im = ax.imshow(
    np.linalg.norm(mfr_2slice_out['metrics']['j'].to_value(u.G / u.s), axis=-1).sum(2).T * Mm_per_pixel * 1e8,
    origin='lower', extent=extent, norm=j_norm, cmap='inferno')
ax.set_title('NF2 - multi height')
divider = make_axes_locatable(ax)
cax = divider.append_axes('right', size='5%', pad=0.05)
...
ax = axs[0]
ax.imshow(np.linalg.norm(mfr_1slice_out['metrics']['j'][y_slice, :, :].to_value(u.G / u.s), axis=-1).T, origin='lower',
          extent=extent, norm=b_norm, cmap='inferno')
ax.set_title('NF2 - single height')
divider = make_axes_locatable(ax)
cax = divider.append_axes('right', size='5%', pad=0.05)
cax.set_axis_off()
...
ax = axs[1]
im = ax.imshow(np.linalg.norm(mfr_2slice_out['metrics']['j'][y_slice, :, :].to_value(u.G / u.s), axis=-1).T,
               origin='lower', extent=extent, norm=b_norm, cmap='inferno')
ax.set_title('NF2 - multi height')
divider = make_axes_locatable(ax)
cax = divider.append_axes('right', size='5%', pad=0.05)
