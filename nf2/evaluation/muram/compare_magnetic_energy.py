...
if __name__ == '__main__':
    ...
    nf2_magnetostatic_outputs = nf2_magnetostatic__model.load_cube(Mm_per_pixel=Mm_per_pixel, metrics=['j'],
                                                                   progress=True, height_range=[0, 20])

    nf2_force_free_model = CartesianOutput(args.nf2_path_force_free)
    nf2_force_free_outputs = nf2_force_free_model.load_cube(Mm_per_pixel=Mm_per_pixel, metrics=['j'],
                                                            progress=True, height_range=[0, 20])
    ...
    metrics_magnetostatic = compute_metrics(nf2_ms_b[:, :, :-1], muram_B)
    metrics_force_free = compute_metrics(nf2_ff_b[:, :, :-1], muram_B)
