...
if __name__ == '__main__':
    ...
    parser.add_argument('--nf2_path_magnetostatic', type=str, help='path to the source NF2 files for the magnetostatic model')
    ...
    muram_out = snapshot.load_base(resolution=args.Mm_per_pixel * u.Mm / u.pix, base_height=116)
    ...
    nf2_magnetostatic_outputs = nf2_magnetostatic__model.load_cube(Mm_per_pixel=Mm_per_pixel, metrics=['j'], progress=True)
    ...
    ax.plot(magnetic_energy_nf2_magnetostatic.sum((0, 1)) * (Mm_per_pixel * 1e8) ** 2,
            np.arange(magnetic_energy_nf2_magnetostatic.shape[2]) * (Mm_per_pixel * 1e8), label='NF2 magnetostatic', linestyle='dashed')
    ax.plot(magnetic_energy_nf2_force_free.sum((0, 1)) * (Mm_per_pixel * 1e8) ** 2,
            np.arange(magnetic_energy_nf2_force_free.shape[2]) * (Mm_per_pixel * 1e8), label='NF2 force-free', linestyle='dotted')
    ...
    ax.plot(free_energy_nf2_magnetostatic.sum((0, 1)) * (Mm_per_pixel * 1e8) ** 2,
            np.arange(free_energy_nf2_magnetostatic.shape[2]) * (Mm_per_pixel * 1e8), label='NF2 magnetostatic', linestyle='dashed')
    ax.plot(free_energy_nf2_force_free.sum((0, 1)) * (Mm_per_pixel * 1e8) ** 2,
            np.arange(free_energy_nf2_force_free.shape[2]) * (Mm_per_pixel * 1e8), label='NF2 force-free', linestyle='dotted')
    ...
    metrics_magnetostatic = compute_metrics(nf2_magnetostatic_outputs['b'].to_value(u.G), muram_B)
    metrics_force_free = compute_metrics(nf2_force_free_outputs['b'].to_value(u.G), muram_B)
    ...
    ax = axs[0]
    ax.plot(metrics_magnetostatic['E_n'], np.arange(metrics_magnetostatic['E_n'].shape[0]) * Mm_per_pixel, label='NF2 magnetostatic', linestyle='dashed')
    ax.plot(metrics_force_free['E_n'], np.arange(metrics_force_free['E_n'].shape[0]) * Mm_per_pixel, label='NF2 force-free', linestyle='dotted')
    ax.set_xlabel(r"$E_\text{n}'$")

    ax = axs[1]
    ax.plot(metrics_magnetostatic['c_cs'], np.arange(metrics_magnetostatic['c_cs'].shape[0]) * Mm_per_pixel, label='NF2 magnetostatic', linestyle='dashed')
    ax.plot(metrics_force_free['c_cs'], np.arange(metrics_force_free['c_cs'].shape[0]) * Mm_per_pixel, label='NF2 force-free', linestyle='dotted')
    ax.set_xlabel(r'$c_\text{cs}$')

    ax = axs[2]
    ax.plot(metrics_magnetostatic['c_vec'], np.arange(metrics_magnetostatic['c_vec'].shape[0]) * Mm_per_pixel, label='NF2 magnetostatic', linestyle='dashed')
    ax.plot(metrics_force_free['c_vec'], np.arange(metrics_force_free['c_vec'].shape[0]) * Mm_per_pixel, label='NF2 force-free', linestyle='dotted')
    ax.set_xlabel(r'$c_\text{vec}$')

    ax = axs[3]
    ax.plot(metrics_magnetostatic['E_m'], np.arange(metrics_magnetostatic['E_m'].shape[0]) * Mm_per_pixel, label='NF2 magnetostatic', linestyle='dashed')
    ax.plot(metrics_force_free['E_m'], np.arange(metrics_force_free['E_m'].shape[0]) * Mm_per_pixel, label='NF2 force-free', linestyle='dotted')
    ax.set_xlabel(r"$E_\text{m}'$")

    ax = axs[4]
    ax.plot(metrics_magnetostatic['eps'], np.arange(metrics_magnetostatic['eps'].shape[0]) * Mm_per_pixel, label='NF2 magnetostatic', linestyle='dashed')
    ax.plot(metrics_force_free['eps'], np.arange(metrics_force_free['eps'].shape[0]) * Mm_per_pixel, label='NF2 force-free', linestyle='dotted')
